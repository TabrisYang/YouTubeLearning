...
你：開課
系統：想學什麼主題？        → 你：技術分析
系統：❓ 你想學的是哪一塊？K線量價、指標、還是型態？
     💡 建議：從 K 線與量價結構開始（一次一題，最多 5 題）→ 你：K線和量價
系統：📋 學習契約確認（主題範圍/語言/難度/片長/時效/頻道…可再修改）→ 你：確認
系統：幾堂課？              → 你：5
...
| 指令 | 功能 |
|---|---|
| `開課` | 逐步引導：主題 → **grill 式訪談收斂「學習契約」**（一次一題附建議，最多 5 題）→ 堂數 → 確認 |
| `開課 <主題> <堂數>` | 一行式（熟手用） |
...
**學習契約**：grill 式開課訪談收斂出的規格（主題含/不含、語言、起始難度、片長、
時效、頻道黑白名單、教學形式）——搜尋、粗篩、評估、編排全部服從它
**硬篩**：時長 4–45 分鐘（契約可調）、觀看數 ≥500、契約排除頻道剔除
**評分**：按讚率 35% ＋ 觀看/訂閱比 25% ＋ 新鮮度 25%（兩年內滿分）＋ 語言 15%（繁中滿分、簡中 0.4）；
財經投資等「高流量≠高品質」領域自動降權流量訊號（15%/10%/45%/30%），品質交給 AI 實看判斷
**多樣性**：同頻道上限可調（預設不限，建議設 2）
**AI 評估**：逐支判斷 difficulty(1-5)、quality_score(0-10)、是否過時、
**契約吻合度 contract_fit(0-10)**、**是否導流/賣課片（is_promotional，直接剔除）**
**AI 編排**：依知識依賴排序（難度從契約起點遞增、禁止下降）、同主題去重、湊不滿堂數時誠實回報不灌水
