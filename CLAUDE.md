...
- ✅ **品質翻修（2026-08 pivot）**：grill 式開課訪談 → 學習契約（`LearningContract`）
  貫穿搜尋/粗篩/評估/編排；導流片識別；污染領域流量降權
- ⬜ **D4 品質閘門重驗**：用「技術分析基礎」走完 grill 開課生成課綱，
  創辦人自己願付 NT$60 才算過 —— 不過關回頭修提示詞，禁止部署
- ⬜ **部署上線**（`./deploy.sh` + LINE 官方帳號，約 1 小時）
- ⬜ **創辦人 14 天每日打卡驗證** → 累積抱怨清單，再決定 PWA vs LINE 與商轉
- ⬜ **真人驗證**：10 個使用者走完流程 → 問付費意願（比再加功能重要）

### Backlog：「課程記憶」（部署後才做——依賴 Firestore 的跨課程歷史）

同一使用者持續開課的四個缺口，打包成一個功能：
1. 重複影片：排除清單從「直屬上一門」擴大到「該使用者所有歷史課程」（~10 行）
2. 同主題再開課：grill 訪談加一題「同級複習（換影片）還是往上加深？」
3. 連續進階：沿 advanced_from 鏈累積排除整條系譜；難度起點踩上一輪最高難度
4. 課程系譜比對：新契約 vs 歷史契約做便宜 LLM 相似度比對，重疊時攤牌問
   接續進階／同級複習／獨立新課（學習契約結構化後這件事才變可行）

品質判斷的長期方向：讚/爛回饋累積成頻道信譽分回流選片（資料護城河）；
評估 rubric 化（實例演示、可驗證性、引用片段佐證）；同批相對排名取代絕對打分。

> 工程紀律：驗證有結果之前，**只修 bug 不加功能**。
> （2026-08 pivot 的依據：創辦人自己實測 D4 閘門不及格 —— 「找錯方向、選片爛、
> 排序亂」三病並存，屬於「驗證已有結果、結果是品質不過關」，因此翻修品質
> 是遵守紀律而非破戒。新功能仍然凍結，直到 D4 重驗過關 + 14 天打卡有結論。）
